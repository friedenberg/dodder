...
type Flag struct {
	...
	Value    interfaces.FlagValue // value as set
...
func (f *FlagSet) set(name, value string) error {
	...
	if !ok {
		...
		return errors.BadRequestf("no such flag -%v", name)
	}
...
func isZeroValue(flag *Flag, value string) (ok bool, err error) {
	...
	return value == z.Interface().(interfaces.FlagValue).String(), nil
}
...
func (f *FlagSet) Var(value interfaces.FlagValue, name string, usage string) {
...
// would
// decompose the comma-separated string into the slice.
func Var(value interfaces.FlagValue, name string, usage string) {
...
func (f *FlagSet) parseOne() (bool, error) {
	...
	if len(name) == 0 || name[0] == '-' || name[0] == '=' {
		return false, errors.BadRequestf("bad flag syntax: %q", s)
	}
	...
	if !ok {
		if name == "help" ||
			name == "h" { // special case for nice help message.
			f.usage()
	...
	if fv, ok := flag.Value.(boolFlag); ok &&
		fv.IsBoolFlag() { // special case: doesn't need an arg
		if hasValue {
			...
			if err := fv.Set("true"); err != nil {
				return false, errors.BadRequestf("invalid boolean flag %s: %v", name, err)
			}
		}
	} else {
		...
		if err := flag.Value.Set(value); err != nil {
			...
			if errors.As(err, &errInvalidValue) {
